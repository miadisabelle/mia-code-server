# Prompt Decomposition

## Directions

- 🌅 **EAST** — VISION: What is being asked?
- 🔥 **SOUTH** — ANALYSIS: What needs to be learned?
- 🌊 **WEST** — VALIDATION: What needs reflection?
- ❄️ **NORTH** — ACTION: What executes the cycle?

## Original Prompt

> Evolve the mia-code-server RISE Framework rispecs by integrating key concepts from the GitHub Copilot Workspace platform specification. The Copilot Workspace had a Task→Spec→Plan→Code pipeline with steerable AI at every stage, workspace session management, repository indexing, and PR creation workflow. The existing rispecs have 62 specs across 8 categories (mia-server-core, miaco-module, three-universe, agentic-ide, narrative-intelligence, mia-vscode, codevops-platform, pde-engine).
>
> Key evolution areas:
> 1. Create new rispecs that capture the Workspace Lifecycle concept (idea→spec→plan→code→PR as a first-class entity), recognizing that CW's "Current State vs Proposed State" specification IS essentially RISE's Structural Tension Chart made interactive and steerable

* Copilot Workspace was considered the first Agentic System actually coding with you...

> 2. Evolve the PDE engine specs to include steerable decomposition where users can edit/regenerate at any phase

* Makes me see that what Copilot Workspace was doing was sort of the PDE that you just used (like this first phase that starts from either your input (probably llms/llms-creative-orientation.txt talks about the first phase of the creative process which is "germination")
* * Either we use our current PDE or we enhance it given what copilot workspace was doing that inspires us...
* * Probably @llms/llms-rise-framework.txt will be usefull in that matter to try to think of how the initial input interact with the RISE framework specs or become one....




> 3. Enhance the codevops-platform specs with a Spec→Plan→Code pipeline that integrates with the existing workflow engine and narrative system

* Indeed, my previous notes on RISE Framework as part of that is relevant...

> 4. After creating/evolving rispecs, choose the most valuable core implementation for a proof-of-concept and build it as a working TypeScript module within mia-code-server
>
> The POC should be a Workspace Lifecycle Engine that:
> - Manages workspace sessions (create, list, resume, complete)
> - Implements the Task→Spec→Plan→Code phases as narrative arcs
> - Integrates with STC charts (Current Reality = Current State, Desired Outcome = Proposed State)
> - Provides HTTP API endpoints for each phase
> - Is testable with unit tests
> - Runs within the mia-code-server Node.js infrastructure

## Primary Intent

**Action:** evolve
**Target:** mia-code-server RISE Framework rispecs by integrating Copilot Workspace concepts and building a Workspace Lifecycle Engine POC
**Urgency:** session
**Confidence:** 95%

## Secondary Intents

1. **create** — new rispecs capturing Workspace Lifecycle concept (idea→spec→plan→code→PR) _(explicit)_
2. **recognize** — CW's Current State vs Proposed State as RISE's Structural Tension Chart made interactive _(explicit)_
3. **evolve** — PDE engine specs to include steerable decomposition with user editing at any phase _(explicit)_
   - depends on: create new workspace lifecycle rispecs
4. **enhance** — codevops-platform specs with Spec→Plan→Code pipeline integrated with workflow engine _(explicit)_
   - depends on: create new workspace lifecycle rispecs
5. **build** — Workspace Lifecycle Engine POC as working TypeScript module _(explicit)_
   - depends on: evolve rispecs
6. **implement** — workspace session management (create, list, resume, complete) _(explicit)_
   - depends on: build POC
7. **implement** — Task→Spec→Plan→Code phases as narrative arcs _(explicit)_
   - depends on: workspace session management
8. **integrate** — STC charts with workspace phases (Current Reality = Current State, Desired Outcome = Proposed State) _(explicit)_
   - depends on: Task→Spec→Plan→Code phases
9. **create** — HTTP API endpoints for each workspace phase _(explicit)_
   - depends on: workspace lifecycle phases
10. **write** — unit tests for the workspace lifecycle engine _(explicit)_
   - depends on: HTTP API endpoints
11. **ensure** — POC runs within mia-code-server Node.js infrastructure _(implicit)_
   - depends on: build POC

## Context Requirements

### Files Needed
- rispecs/codevops-platform/*.spec.md
- rispecs/pde-engine/*.spec.md
- rispecs/mia-server-core/*.spec.md
- rispecs/miaco-module/04-stc-charts.spec.md
- rispecs/README.md
- /a/src/IAIP/prototypes/artefacts/MiAICo-251203-CopilotWorkspace-N0v2akS7SXyFCRiSpAxDNQ/copilot-workspace-specs.md
- src/node/routes/index.ts
- package.json
- tsconfig.json

### Tools Required
- file-create
- file-edit
- typescript-compiler
- test-runner
- git

### Assumptions
- CW's Current State vs Proposed State specification IS RISE's Structural Tension Chart made interactive
- The existing 62 rispecs provide sufficient foundation for evolution
- mia-code-server Node.js infrastructure can host new modules
- TypeScript is the implementation language
- The POC should be self-contained enough to demonstrate the core workflow

## Four Directions Analysis

### 🌅 EAST — VISION

- Recognize CW's Spec generation as interactive STC — unifying two conceptual frameworks [95%]
- Vision: Workspace Lifecycle as first-class entity spanning idea to PR [95%]
- Vision: Steerable AI at every phase of the development lifecycle [90%]

### 🔥 SOUTH — ANALYSIS

- Analyze existing 62 rispecs for evolution opportunities [90%]
- Study CW data models (Workspace, Task, Spec, Plan, Implementation interfaces) [85%]
- Understand how STC charts map to CW's Current/Proposed state pattern [90%]

### 🌊 WEST — VALIDATION

- Write unit tests for workspace lifecycle engine [90%]
- Validate POC runs within Node.js infrastructure [85%] _(implicit)_
- Review evolved rispecs for consistency with RISE framework [80%] _(implicit)_

### ❄️ NORTH — ACTION

- Create 4 new rispecs (workspace-lifecycle, spec-plan-code-pipeline, steerable-decomposition, workspace-api) [95%]
- Build Workspace Lifecycle Engine TypeScript module [95%]
- Implement HTTP API endpoints for workspace phases [90%]
- Implement STC chart integration bridge [85%]

## Action Stack

- [ ] Analyze existing rispecs and CW specs for evolution mapping
- [ ] Create rispecs/codevops-platform/07-workspace-lifecycle.spec.md — Workspace as first-class entity with Task→Spec→Plan→Code→PR phases
- [ ] Create rispecs/codevops-platform/08-spec-plan-code-pipeline.spec.md — Pipeline engine integrating with workflow engine (depends on: workspace-lifecycle spec)
- [ ] Create rispecs/pde-engine/05-steerable-decomposition.spec.md — User-editable decomposition with regeneration at any phase
- [ ] Create rispecs/mia-server-core/11-workspace-api.spec.md — REST API endpoints for workspace CRUD (depends on: workspace-lifecycle spec)
- [ ] Update rispecs/README.md with new spec entries (depends on: all new rispecs created)
- [ ] Design TypeScript types for Workspace, Phase, STC-bridge interfaces (depends on: rispecs complete)
- [ ] Implement WorkspaceStore — filesystem-based workspace persistence (depends on: types designed)
- [ ] Implement WorkspaceLifecycleEngine — phase transitions, STC integration, narrative arc tracking (depends on: store implemented)
- [ ] Implement STC Bridge — maps workspace phases to structural tension charts (depends on: engine implemented)
- [ ] Implement HTTP routes — REST endpoints for workspace operations (depends on: engine implemented)
- [ ] Write unit tests for store, engine, and routes (depends on: routes implemented)
- [ ] Run tests and validate POC works within mia-code-server infrastructure (depends on: tests written)

## Ambiguity Flags

- **"Whether the POC should actually call an LLM for spec/plan/code generation or use stub/mock generation"**
  - Suggestion: Start with structured data management and mock AI generation — real LLM integration is a separate concern
- **"How deeply the STC integration should go — simple mapping or full chart lifecycle management"**
  - Suggestion: Implement the conceptual bridge (workspace phase ↔ STC chart) with simple chart creation, defer full lifecycle
- **"Whether HTTP routes should be mounted in the existing code-server Express app or run standalone"**
  - Suggestion: Design as Express Router that CAN be mounted but is also testable standalone

## Expected Outputs

### Artifacts
- rispecs/codevops-platform/07-workspace-lifecycle.spec.md
- rispecs/codevops-platform/08-spec-plan-code-pipeline.spec.md
- rispecs/pde-engine/05-steerable-decomposition.spec.md
- rispecs/mia-server-core/11-workspace-api.spec.md
- src/node/mia/workspace-lifecycle/types.ts
- src/node/mia/workspace-lifecycle/engine.ts
- src/node/mia/workspace-lifecycle/store.ts
- src/node/mia/workspace-lifecycle/routes.ts
- src/node/mia/workspace-lifecycle/stc-bridge.ts
- src/node/mia/workspace-lifecycle/index.ts
- test/unit/mia/workspace-lifecycle/engine.test.ts
- test/unit/mia/workspace-lifecycle/store.test.ts
- test/unit/mia/workspace-lifecycle/routes.test.ts

### Updates
- rispecs/README.md
